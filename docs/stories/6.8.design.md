...
> **Baseline note.** The table above is the recon at `3990a58` (pre-implementation). **T01 has since landed** (`1770255`, `5e7565f`) — `total`, keyset `cursor` and per-type filtering are in, **but its privacy predicate implements the superseded rule** (`not:'private' OR IS NULL`) and must be revised to the default-aware, `'public'`-only rule in D2/§14 (see §13 T01 "REVISION REQUIRED").
...
| G2b | Feed privacy rule **collapses the three-valued enum** (`public`/`friends`/`private`) to two: `not:'private'` publishes `'friends'` content to a viewer-agnostic feed; and a blanket "absent key = public" contradicts each key's **own default** (`stats_share` defaults to `'friends'`) | P0 **privacy leak** | T01 (revise) |
...
| **D2** | Per-type privacy, keyed **and default-aware**. Filter each share by its own key — `session_share` for `type='session'`, `stats_share` for `'match'`, `achievements_share` for `'achievement'` — and a share is visible **only if the *effective* value of its governing key is `'public'`**, where effective = the stored value, or the key's **own write-path default** when absent (`getPrivacySettings`, `sharingService.ts:347-351`). Per branch: `session` → `= 'public' OR IS NULL` (default `public`); `match` → `= 'public'` **with no `IS NULL` branch** (default `friends`); `achievement` → `= 'public' OR IS NULL` (default `public`). | G2 + G2b. The privacy enum is **three**-valued (`'public' \| 'friends' \| 'private'`, `routes/sharing.ts:32-34`); `not:'private'` would publish `'friends'` content, and a blanket "absent = public" would publish `stats_share` content whose default is `'friends'`. Prisma JSON `path` filters cannot be parameterised by a column, so the fix is **one `findMany` with an `OR` of three `{ type, sharer }` branches** — no raw SQL (SQL-injection surface + no test parity). |
...
- `shares`: `where` = OR of the three per-type privacy branches (D2 — **default-aware, `'public'`-only**; the `match`/`stats_share` branch has **no** absent/`NULL` branch because its default is `'friends'`), `orderBy: [{createdAt:'desc'},{id:'desc'}]`, `take: limit`, and either `skip: offset` (no cursor) or `cursor` keyset predicate.
...
| 13 | Privacy respected | T01 (revise), T05 | ✅ — **AC 13 is exactly this requirement.** The privacy enum is **three**-valued; a share is visible in the global feed only if its governing key's **effective** value is `'public'` (D2). This covers both the per-type leak (G2) and the `'friends'`/default collapse (G2b). |
...
| Q2 | Should the feed include friend-only (`'friends'`) shares for the viewer? | **No — not now, and the old "as today" framing is untenable.** The feed is **viewer-agnostic** (`getCommunityFeed(userId?, …)` never uses `userId` — see its JSDoc *"reserved; the feed is public today"*), and there is **zero** friend-scoping in the query. A public, viewer-agnostic feed **cannot** legitimately surface `'friends'` content, so under D2 it is **hidden** (it is still *creatable* — the write path only blocks `'private'`, `sharingService.ts:111`). **Friend-scoping is the future enhancement that would make `'friends'` meaningful** (join `Friend`/`SocialConnection` against the viewer); until it exists, `'friends'` means "not in the global feed". |
...
- **Note:** the per-type `OR` privacy filter (D2) is the load-bearing change.
- **REVISION REQUIRED (post-`5e7565f`).** T01 has landed (`1770255`, `5e7565f`) but its predicate `privacyVisibilityFor(key)` = `not:'private' OR IS NULL` implements the **superseded** rule. It must be replaced with the **default-aware, `'public'`-only** rule in D2/§14: `session`/`achievement` keep the `IS NULL` branch; **`match` drops it** and matches `equals:'public'` only. Suggested shape: `privacyVisibilityFor(key, defaultIsPublic)` → `equals:'public'` plus the `DbNull` branch **only when `defaultIsPublic`**; call with `('session_share', true)`, `('stats_share', false)`, `('achievements_share', true)`. **Test matrix (all three types):** `'public'` → visible; `'friends'` → hidden; `'private'` → hidden; absent/`NULL` → visible for `session`/`achievement`, **hidden for `match`**.
...
- **Privacy.** The privacy value is a **three-valued** enum — `'public' | 'friends' | 'private'` (`routes/sharing.ts:32-34`). **Do not collapse it to two.** A share is visible in the global feed **iff the *effective* value of its governing key is `'public'`**, where *effective* = the stored value, or the key's **own write-path default** (`getPrivacySettings`, `sharingService.ts:347-351`) when the key is absent/`NULL`:

  | Share `type` | Governing key | Visible when | Write-path default |
  |---|---|---|---|
  | `session` | `session_share` | `= 'public'` **or** key absent/`NULL` | `'public'` |
  | `match` | `stats_share` | `= 'public'` — **no absent/`NULL` branch** | **`'friends'`** |
  | `achievement` | `achievements_share` | `= 'public'` **or** key absent/`NULL` | `'public'` |

  So `'friends'` content is **hidden** from the (viewer-agnostic) feed, and an **absent `stats_share` is hidden** because its default is `'friends'` — *not* because it is `private`. A blanket "absent = public" rule is **wrong** (`stats_share`); a blanket "not `private` = visible" rule is **wrong** (publishes `'friends'`). Only `'public'` is published. This is **AC 13**, not a footnote.

  > **Deliberate behaviour change (recorded, not silent).** Relative to both the pre-T01 code and the T01 commit `5e7565f`, shares whose governing key is `'friends'` — or, for `match`, absent (default `'friends'`) — **become hidden**. This is the privacy-safe direction and is intended.
