...
| **D5** | Repair the CSAT write path: resolve identity via `resolveIdentity(req)` (`middleware/permissions`), resolve the caller's **`MvpPlayer.id` from tournament participation**, validate `rating ∈ [1,5]`, return the `{ success, data }` envelope. | G5. `User.id` → `MvpPlayer.id` FK is the core defect. |
...
**CSAT repair** — `POST /api/v1/tournaments/:id/feedback` (`tournament-analytics.ts:152`):
1. `identity = resolveIdentity(req)`; reject anonymous (feedback is authenticated-only today).
2. Resolve the participant: `prisma.tournamentPlayer.findFirst({ where: { tournamentId, deviceId: identity.deviceId ?? … } })` **or** join through `MvpPlayer.userId === identity.userId` — take the real **`MvpPlayer.id`**, never `User.id`.
3. Validate `rating ∈ [1,5]` (restore the disabled schema).
4. Return `{ success: true, data: feedback }`.
5. Remove the `'temp-user-id'` fallback entirely (it guarantees an FK violation).
...
Prisma additions (additive only, D4/D6):
...
`Share` gains `reports ShareReport[]`. Both models are **new tables** — no column on an existing table changes type or nullability.
...
| Q4 | Does `resolveIdentity` expose `deviceId` for authenticated users? | Assumed yes for the participant lookup in D5; if not, fall back to a `MvpPlayer.userId` join. |
...
### T02 — NPS instrument + CSAT write-path repair + additive migration
- **Files:** `backend/prisma/schema.prisma` (add `NpsResponse`, `ShareReport`; `Share.reports`), `backend/prisma/migrations/<ts>_story_6_8_community/migration.sql` (**new**, additive), `backend/src/routes/tournament-analytics.ts` (repair `/:id/feedback`), `backend/src/services/communityService.ts` (NPS create + summary), `backend/src/routes/community.ts` (`POST /nps`, `GET /nps/summary`), `backend/src/routes/__tests__/nps.test.ts` + `.../feedback.identity.test.ts` (**new**).
- **ACs:** 3, 8, 10 (instrument). **Deps:** —. **Priority:** P1.
- **Note:** both new tables live in **one** migration so there is a single migration artifact to review.
...
- **Identity.** Use `resolveIdentity(req)` (`middleware/permissions`) — it yields `{ source:'jwt'|'device'|'none', userId?, deviceId? }` and is the **only** sanctioned resolver. Never read `req.user.name` (it does not exist). `TournamentFeedback.playerId` **FKs to `MvpPlayer.id`**, never `User.id`.
...
- **Additive-only schema.** New tables only; no type/nullability change on existing columns.
