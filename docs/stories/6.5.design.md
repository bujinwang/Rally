...
**Stray file worth removing:** `backend/prisma/schema.prisma.bak` is untracked,
**not gitignored**, and stale (it predates many live fields such as `costModel`,
`birdie*`, `visibility: 'club'`). A future `git add -A` would commit it. Delete
it or add it to `.gitignore`.
