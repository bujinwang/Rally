...
| 11 | **`TournamentMatch.player1Id`/`player2Id` are NOT NULL with FKs** → placeholder/bye slots impossible | ✅ confirmed | schema `:918-921`; live pg: `player1Id|NO`, `player2Id|NO`, `roundId|YES`, `winnerId|YES` |
...
2. **AC 3 "persists to TournamentRound/TournamentMatch/TournamentGame"** — impossible as written: player slots are NOT NULL (finding 11). Needs an additive relaxation.
...
- `validateBracket(bracket)` → **rewritten** as a real invariant checker (it currently has a bug: `validateBracket` uses `player1Id`/`player2Id` as required, and its "advancing players" check compares raw counts) and used both as a runtime post-generation assertion and as a test oracle.
...
Today `updateMatchResult` advances by picking "the first match with an empty slot" — which **cannot work** because slots are NOT NULL (finding 11) and, with `tournamentService.generateBracket`, are pre-filled with wrong players, so the winner is silently dropped. Incremental advancement is the corruption vector the story names as its primary risk.

**Decision: make progression a pure *projection* of recorded results.**
- `projectBracket(tournamentId)`: within a transaction, read all matches, clear every non-first-round slot, then walk rounds in order and place each match's winner into the slot designated by its feed link (`feedMatch1Id`→`player1`, `feedMatch2Id`→`player2`); auto-advance byes; mark round/tournament status.
...
### D6 — AC 15 permissions: a **tournament** organizer guard (firm)

`requireOrganizer` is session-scoped and unusable here. Add `middleware/tournamentPermissions.ts` → `requireTournamentOrganizer` that:
- resolves identity via the existing `resolveIdentity(req)` (JWT `userId` first, else `deviceId`);
- authorizes when the identity matches the tournament's `organizerUserId` **or** `organizerDeviceId` (new nullable columns, §2), else 403 in the standard `{success:false,error:{code,message}}` envelope;
- is applied **only to the new mutation endpoints**; existing public endpoints are untouched (preserves AC 5 contracts).

Because `Tournament.organizer` is free text (finding 13) it is **not** used for authorization. Additive `organizerUserId`/`organizerDeviceId` are populated on create when an identity is present.
...
## 2. Schema changes (additive only — AC 14)

All changes are relaxations or new nullable columns; none drop or retype existing columns, so existing rows/contracts are safe.

| Model | Change | Why |
|-------|--------|-----|
| `TournamentMatch` | `player1Id String` → `String?`; `player1 …` → `TournamentPlayer?` (same for `player2`) | Placeholder slots for future rounds & byes (finding 11). **Required for AC 3.** |
| `TournamentMatch` | add `feedMatch1Id String?`, `feedMatch2Id String?` (nullable self-relation, named) | Deterministic winner placement for projection/correction (D5). |
| `TournamentMatch` | add `correctedAt DateTime?`, `correctionReason String?` | AC 12 audit trail. |
| `Tournament` | add `organizerUserId String?`, `organizerDeviceId String?` | AC 15 authorization (D6); `organizer` stays for back-compat. |
| `TournamentPlayer` | add `userId String?` | AC 8 forward path (nullable; unused until identity exists). |
| `TournamentRound` | add `@@index([tournamentId, roundNumber])` (already present) — no change | — |

Migration is a single hand-written additive migration; `prisma generate` + `tsc --noEmit` must stay clean.
...
- `backend/src/services/bracket/types.ts` — canonical domain types + enums
- `backend/src/services/bracket/engine.ts` — pure generators (single/double/RR/Swiss), seeded PRNG, bye placement, `getRoundName`, `validateBracket`, `projectBracket`
- `backend/src/services/bracket/persistence.ts` — `persistBracket` (batch in `$transaction`), `getBracketState`, `applyResult`, `correctResult`, `clearDerivedDownstream`
- `backend/src/services/bracket/index.ts` — barrel
- `backend/src/middleware/tournamentPermissions.ts` — `requireTournamentOrganizer`
...
**Modified**
- `backend/src/services/tournamentBracketService.ts` — becomes a thin facade delegating to `bracket/*`; **keeps** its exported names (`BracketMatch`, `TournamentBracket`, `BracketGenerationOptions`, `generateBracket`, `updateMatchResult`, `getBracketState`, singleton) so existing imports/tests still compile
- `backend/src/services/tournamentService.ts` — fix `organizer` field defect; `startTournament` calls bracket generation
- `backend/src/services/tournamentAnalyticsService.ts` — call ordering fix (participation before efficiency); wire after result
...
classDiagram
    ...
    class TournamentOrganizerGuard {
      +requireTournamentOrganizer(req, res, next) void
    }
...
export function generateDoubleElimination(o: BracketGenerationOptions): TournamentBracket;
...
// bracket/persistence.ts
export async function persistBracket(b: TournamentBracket): Promise<void>; // rounds+matches via createMany in one $transaction
...
// middleware/tournamentPermissions.ts
export const requireTournamentOrganizer: RequestHandler;                  // reads :id, resolves identity, 403 envelope
```
...
- **The real cost is DB writes.** The existing pattern writes one row per match with sequential `await` (63 round-trips). **Decision:** persist rounds with one `createMany`, then matches with one `createMany`, both inside a single `$transaction` → **2 round-trips** regardless of size. Emission happens **after** the response and is fire-and-forget (already the pattern).
...
1. **Correctness matrix (AC 9):** for n ∈ {2,3,4,5,6,7,8,9,15,16,17,31,32,64}, single elimination produces `ceil(log2 n)` rounds, `n-1` real matches, and **zero BYE-vs-BYE** matches; byes are assigned to the **top seeds**; every non-bye slot is a distinct player; round counts are `2^(R-r)`.
2. **Determinism (AC 13):** same inputs + same `seed` ⇒ deep-equal brackets across two runs; `randomizeSeeding` with a fixed seed is deterministic; **no `Math.random`** appears in `bracket/` (static check).
...
5. **Progression (AC 3, AC 11):** play a full 8-player bracket via `applyResult`; assert champion in `TournamentResult`, `status='COMPLETED'`, `TournamentPlayer.finalRank` set; assert idempotency (re-applying a result is a no-op).
6. **Correction (AC 12):** record a wrong winner mid-bracket, correct it, assert the bracket re-projects, the previously-advanced player is removed downstream, and the true winner advances; assert the `DOWNSTREAM_COMPLETED` guard fires without `cascade`.
...
**Verifiable here:** AC 1, 2, 3, 5, 6 (frontend additive), 7 (emit shapes), 9 (generation correctness on synthetic inputs — deterministic, DB-free), 11, 12, 13, 14, 15, 16.
...
- Deps: T01
- Detail: single/double/RR/Swiss with correct byes (no BYE-vs-BYE), standard seeding, seeded PRNG; `getRoundName`; rewritten `validateBracket`; pure `projectBracket`. Covers AC 1, 2, 9, 13. **This is the highest-risk, most-tested unit.**
...
- Deps: T01, T02
- Detail: `persistBracket` (batch `createMany` in `$transaction`), `getBracketState`, `applyResult`, `correctResult` + downstream clearing, re-emit. Covers AC 3, 12, 16. Delete `bracketService.ts`.
...
- Deps: T01, T03
- Detail: hook `/:id/start`; add `bracket/generate`, `GET bracket`, `matches/:id/result`, `matches/:id/correct`, `standings`; organizer guard on mutations only; analytics call-order fix; preserve existing 9 contracts. Covers AC 4, 5, 7, 15.
...
3. **Double elimination scope** — full LB progression (feed links, LB→WB final reset) is materially more work than single elim. Confirm whether double elimination must be *fully playable* in this story or single elim + RR/Swiss playable with double elim **generation-only** (structure persisted, progression deferred). Recommend: fully playable single elim + RR + Swiss; double elim structure-correct with progression if time allows.
