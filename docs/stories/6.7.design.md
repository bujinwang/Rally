...
- **QA caveat**: QA had no live DB, so DB-touching logic was exercised through a fake Prisma; real Postgres constraint/`$transaction` behaviour was unverified by QA. The architect independently verified the **live** constraint that matters most — `tournament_matches.player1Id/player2Id` are `NOT NULL` (§0 finding 11, psql). T03/T04 tests must therefore exercise the real Prisma path (or a constraint-faithful mock) for the nullable-slot relaxation.
...
Today `updateMatchResult` advances by picking "the first match with an empty slot" — which **cannot work** because slots are NOT NULL (finding 11) and, with `tournamentService.generateBracket`, are pre-filled with wrong players, so the winner is silently dropped. Incremental advancement is the corruption vector the story names as its primary risk. **QA reproduced three concrete corruption modes** by execution (§0.1): (i) the corrected winner is written to the match but the downstream slot keeps the **stale** winner; (ii) **both** next-round slots get filled from the **same** upstream match; (iii) recording the same result **twice** places the **same player in both slots**. Re-recording an already-advanced result does not refuse and does not no-op — it silently corrupts. **Correction is therefore P0, not merely a risk.**
