...
class NotificationService {
   ...
   */
  async registerPushToken(deviceId: string) {
    if (!this.pushToken) {
    ...
    try {
      ...
      if (result.success) {
        console.log('Push token registered:', result.data?.tokenId);
        return true;
      }
  ...
  /**
   * Subscribe to session push notifications
   */
  async subscribeToSession(shareCode: string, deviceId: string) {
    try {
      ...
      const result = await response.json();
      if (result.success) {
        console.log('Subscribed to session notifications:', shareCode);
        return true;
      }
      console.warn('Subscribe to session failed:', result);
      return false;
    } catch (error) {
  ...
  /**
   * Unsubscribe from session push notifications
   */
  async unsubscribeFromSession(shareCode: string, deviceId: string) {
    try {
      ...
      const result = await response.json();
      if (result.success) {
        console.log('Unsubscribed from session notifications:', shareCode);
        return true;
      }
      console.warn('Unsubscribe from session failed:', result);
      return false;
    } catch (error) {
