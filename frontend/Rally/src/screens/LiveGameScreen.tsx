...
export default function LiveGameScreen() {
  ...
  // Game timer: update every second for in-progress games
  useEffect(() => {
    const interval = setInterval(() => {
      const now = Date.now();
      const newElapsed: Record<string, number> = {};
      sessionData?.courts?.forEach((court: any) => {
        const game = court.currentGame;
        if (game && game.startTime && game.status === 'IN_PROGRESS') {
          newElapsed[court.id] = Math.floor((now - new Date(game.startTime).getTime()) / 1000);
        }
      });
  ...
  const renderCourt = (court: Court) => (
    <View key={court.id} style={[styles.courtCard, { backgroundColor: colors.cardBg, borderColor: colors.cardBorder }]}>
      ...
      {/* Game Timer */}
      {court.currentGame && elapsedSeconds[court.id] !== undefined && (
        <View style={styles.timerRow}>
          <Text style={styles.timerIcon}>⏱️</Text>
          <Text style={styles.timerText}>{formatElapsed(elapsedSeconds[court.id])}</Text>
        </View>
      )}
