...
exports.updatePlayerGameStatistics = updatePlayerGameStatistics;
exports.updatePlayerMatchStatistics = updatePlayerMatchStatistics;
exports.getPlayerStatistics = getPlayerStatistics;
exports.getSessionStatistics = getSessionStatistics;
exports.getSessionLeaderboard = getSessionLeaderboard;
const database_1 = require("../config/database");
/**
 * Update player statistics after a game is completed
 */
async function updatePlayerGameStatistics(sessionId, gameData) {
    ...
    await database_1.prisma.mvpPlayer.updateMany({
        where: {
            sessionId,
            name: { in: allPlayers }
        },
 ...
 */
async function updateRestCounters(sessionId) {
 ...
 */
async function updatePlayerMatchStatistics(sessionId, matchData) {
 ...
 */
async function updatePartnershipStatistics(sessionId, player1Name, player2Name, won) {
    const players = await database_1.prisma.mvpPlayer.findMany({
        where: {
            sessionId,
    ...
    for (const player of players) {
        ...
        const currentStats = player.partnershipStats || {};
 ...
 */
async function recalculatePlayerMetrics(sessionId, playerName) {
 ...
 */
async function getPlayerStatistics(sessionId, playerName) {
    const player = await database_1.prisma.mvpPlayer.findFirst({
        where: { sessionId, name: playerName }
    });
    if (!player)
        return null;
    const partnershipStats = player.partnershipStats || {};
    const partnershipRecords = Object.entries(partnershipStats).map(([partnerName, stats]) => ({
        partnerName,
...
async function getSessionStatistics(sessionId) {
    ...
    const mostActivePlayer = session.players.reduce((prev, current) => current.gamesPlayed > prev.gamesPlayed ? current : prev).name;
    ...
    for (const player of session.players) {
        const partnerships = player.partnershipStats || {};
        for (const [partnerName, stats] of Object.entries(partnerships)) {
...
async function getSessionLeaderboard(sessionId) {
    ...
    for (const player of players) {
        const partnershipStats = player.partnershipStats || {};
        for (const [partnerName, stats] of Object.entries(partnershipStats)) {
    ...
    const byPartnership = partnerships
        .filter((p, index, arr) => arr.findIndex(p2 => p2.players[0] === p.players[0] && p2.players[1] === p.players[1]) === index)
        .sort((a, b) => b.winRate - a.winRate)
        .slice(0, 10);
    return {
        byWinRate,
