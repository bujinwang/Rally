...
const aiPairingService_1 = require("../services/aiPairingService");
...
router.post('/sessions/:sessionId/pairings', (0, auth_1.requireRole)(['OWNER', 'ORGANIZER']), (0, validation_1.validate)(validation_1.validatePairingRequest), async (req, res) => {
    try {
        ...
        const pairingResult = await pairingService_1.PairingService.generatePairings(sessionId, algorithm);
...
router.get('/sessions/:sessionId/pairings', async (req, res) => {
    try {
        const { sessionId } = req.params;
        const userId = req.user?.id;
        // Verify user has access to this session
        ...
        const pairingResult = await pairingService_1.PairingService.generatePairings(sessionId, 'fair');
        res.json({
            success: true,
            data: pairingResult,
            timestamp: new Date().toISOString()
...
router.put('/sessions/:sessionId/pairings/:pairingId', (0, auth_1.requireRole)(['OWNER', 'ORGANIZER']), async (req, res) => {
    try {
        ...
        const currentPairings = await pairingService_1.PairingService.generatePairings(sessionId, 'fair');
        const validation = pairingService_1.PairingService.validatePairing(manualPairing, currentPairings.pairings);
        if (!validation.valid) {
            return res.status(400).json({
                success: false,
                error: {
...
router.delete('/sessions/:sessionId/pairings', (0, auth_1.requireRole)(['OWNER', 'ORGANIZER']), async (req, res) => {
    try {
        const { sessionId } = req.params;
        const userId = req.user?.id;
        // Verify user has access to this session
...
router.post('/suggest', (0, auth_1.requireRole)(['OWNER', 'ORGANIZER']), async (req, res) => {
    try {
        ...
        const aiResult = await aiPairingService_1.AIPairingService.generateAISuggestions(sessionId, playerIds, options);
        res.json({
            success: true,
            data: aiResult,
...
router.get('/explain/:suggestionId', async (req, res) => {
    try {
...
router.post('/feedback', async (req, res) => {
    try {
        ...
        // Record the feedback
        await aiPairingService_1.AIPairingService.recordPairingFeedback(sessionId, playerId, partnerId, feedback, aiSuggested);
        res.json({
            success: true,
...
router.post('/update-skills/:sessionId', (0, auth_1.requireRole)(['OWNER', 'ORGANIZER']), async (req, res) => {
    try {
        const { sessionId } = req.params;
        const userId = req.user?.id;
        // Verify user has access to this session
        ...
        // Update skill levels
        await aiPairingService_1.AIPairingService.updatePlayerSkillLevels(sessionId);
        res.json({
            success: true,
