...
router.get('/', async (req, res) => {
    try {
        ...
        const sessions = await database_1.prisma.mvpSession.findMany({
            where: {
...
router.get('/:shareCode', async (req, res) => {
    try {
        ...
        const session = await database_1.prisma.mvpSession.findFirst({
            where: {
                shareCode: shareCode,
...
router.post('/', createSessionValidation, async (req, res) => {
    try {
        ...
        await database_1.prisma.mvpPlayer.create({
            data: {
        ...
        const sessionWithPlayers = await database_1.prisma.mvpSession.findUnique({
            where: { id: session.id },
            include: {
                players: {
...
// Join session
router.post('/join/:shareCode', joinSessionValidation, async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({
        ...
        const player = await database_1.prisma.mvpPlayer.create({
            data: {
                sessionId: session.id,
                name,
                deviceId,
...
router.post('/claim', claimSessionValidation, async (req, res) => {
    try {
        ...
        const isSecretValid = await password_1.PasswordUtils.verifyPassword(secret, session.organizerSecretHash);
        if (!isSecretValid) {
            return res.status(403).json({
                success: false,
                error: {
...
router.put('/players/:playerId/status', (0, permissions_1.requireOrganizerOrSelf)('update_player_status'), async (req, res) => {
    try {
        ...
        const player = await database_1.prisma.mvpPlayer.update({
            where: { id: playerId },
            data: { status }
        });
        res.json({
...
router.put('/:shareCode', rateLimiters.sensitive, (0, permissions_1.requireOrganizer)('edit_session'), async (req, res) => {
    try {
        const { shareCode } = req.params;
        ...
        if (courtCount !== undefined)
            updateData.courtCount = courtCount;
        ...
        if (canSetOwnerDeviceId) {
            ...
            const ownerPlayer = await database_1.prisma.mvpPlayer.findFirst({
                where: {
                    sessionId: session.id,
            ...
            if (ownerPlayer && !ownerPlayer.deviceId) {
                await database_1.prisma.mvpPlayer.update({
        ...
        const updatedSession = await database_1.prisma.mvpSession.update({
            where: { shareCode },
            data: updateData,
            include: {
                players: {
        ...
        try {
            ...
            if (io) {
                ...
                const freshSession = await database_1.prisma.mvpSession.findUnique({
                    where: { shareCode },
                    include: {
                        players: {
                            select: {
...
router.put('/terminate/:shareCode', rateLimiters.sensitive, (0, permissions_1.requireOrganizer)('delete_session'), async (req, res) => {
    try {
        const { shareCode } = req.params;
...
router.delete('/:shareCode/players/:playerId', rateLimiters.sensitive, (0, permissions_1.requireOrganizer)('remove_players'), async (req, res) => {
    try {
        ...
        const player = await database_1.prisma.mvpPlayer.findUnique({
            where: { id: playerId }
        });
        if (!player) {
            return res.status(404).json({
...
router.post('/:shareCode/add-player', rateLimiters.api, (0, permissions_1.requireOrganizer)('add_players'), async (req, res) => {
    try {
        const { shareCode } = req.params;
...
// Create a new game
router.post('/:shareCode/games', (0, permissions_1.requireOrganizer)('generate_pairings'), async (req, res) => {
    try {
        const { shareCode } = req.params;
        ...
        // Create game
        const game = await database_1.prisma.mvpGame.create({
            data: {
                sessionId: session.id,
                gameNumber,
                team1Player1,
...
router.put('/:shareCode/games/:gameId/score', (0, permissions_1.requireOrganizer)('modify_pairings'), async (req, res) => {
    try {
        ...
        const game = await database_1.prisma.mvpGame.findFirst({
        ...
        // Update game
        const updatedGame = await database_1.prisma.mvpGame.update({
            where: { id: gameId },
            data: {
                team1FinalScore,
                team2FinalScore,
        ...
        await (0, statisticsService_1.updatePlayerGameStatistics)(session.id, {
        ...
        try {
            const { io } = await Promise.resolve().then(() => __importStar(require('../server')));
            const updatedSession = await database_1.prisma.mvpSession.findFirst({
                where: { shareCode },
                include: {
                    players: {
                        select: {
...
router.put('/:shareCode/games/:gameId/teams', (0, permissions_1.requireOrganizer)('modify_pairings'), async (req, res) => {
    try {
        const { shareCode, gameId } = req.params;
        ...
        const sessionPlayers = await database_1.prisma.mvpPlayer.findMany({
            where: {
                sessionId: session.id,
...
router.get('/:shareCode/rotation', async (req, res) => {
    try {
        ...
        const matchHistory = session.matches.map((m) => ({
            id: m.id,
        ...
        const rotationResult = (0, rotationAlgorithm_1.generateOptimalRotation)(players, gameHistory, courts, matchHistory);
        const explanation = (0, rotationAlgorithm_1.getRotationExplanation)(rotationResult.suggestedGames, rotationResult.fairnessMetrics);
        res.json({
            success: true,
            data: {
...
router.post('/:shareCode/matches', (0, permissions_1.requireOrganizer)('generate_pairings'), async (req, res) => {
    try {
        const { shareCode } = req.params;
        ...
        // Create match
        const match = await database_1.prisma.mvpMatch.create({
            data: {
                sessionId: session.id,
                matchNumber,
                team1Player1,
...
router.post('/:shareCode/matches/:matchId/games', (0, permissions_1.requireOrganizer)('generate_pairings'), async (req, res) => {
    try {
        ...
        const match = await database_1.prisma.mvpMatch.findFirst({
            where: {
                id: matchId,
                sessionId: session.id,
                status: 'IN_PROGRESS'
        ...
        const totalGames = await database_1.prisma.mvpGame.count({
...
router.put('/:shareCode/matches/:matchId/games/:gameId/score', (0, permissions_1.requireOrganizer)('modify_pairings'), async (req, res) => {
    try {
        ...
        const matchWinner = isMatchComplete
            ...
            : undefined;
        // Update match
        const updatedMatch = await database_1.prisma.mvpMatch.update({
            where: { id: matchId },
            data: {
        ...
        if (isMatchComplete) {
            await (0, statisticsService_1.updatePlayerMatchStatistics)(session.id, {
                team1Player1: match.team1Player1,
                team1Player2: match.team1Player2,
                team2Player1: match.team2Player1,
                team2Player2: match.team2Player2,
...
router.get('/:shareCode/players/:playerName/stats', async (req, res) => {
    try {
        ...
        const playerStats = await (0, statisticsService_1.getPlayerStatistics)(session.id, playerName);
        if (!playerStats) {
            return res.status(404).json({
                success: false,
...
router.get('/:shareCode/statistics', async (req, res) => {
    try {
        ...
        const sessionStats = await (0, statisticsService_1.getSessionStatistics)(session.id);
        res.json({
            success: true,
            data: {
                sessionStats
...
router.get('/:shareCode/leaderboard', async (req, res) => {
    try {
        ...
        const leaderboard = await (0, statisticsService_1.getSessionLeaderboard)(session.id);
        res.json({
            success: true,
            data: {
                leaderboard
...
router.put('/:shareCode/players/:playerId/status', (0, permissions_1.requireOrganizerOrSelf)('update_player_status'), async (req, res) => {
    try {
        ...
        // Find the player
        const player = await database_1.prisma.mvpPlayer.findFirst({
        ...
        const activeGames = session.games.filter(game => game.team1Player1 === player.name ||
        ...
        // Update player status
        const updatedPlayer = await database_1.prisma.mvpPlayer.update({
            where: { id: playerId },
            data: {
                status: status,
...
router.get('/:shareCode/players/me/:deviceId', async (req, res) => {
    try {
        ...
        const activeGame = session.games.find(game => game.team1Player1 === player.name ||
            game.team1Player2 === player.name ||
            game.team2Player1 === player.name ||
            game.team2Player2 === player.name);
        res.json({
...
router.put('/:shareCode/players/:playerId/status', (0, permissions_1.requireOrganizerOrSelf)('update_player_status'), async (req, res) => {
    try {
        ...
        // Check if player is currently in an active game
        const isInActiveGame = session.games.some(game => game.team1Player1 === player.name ||
            game.team1Player2 === player.name ||
            game.team2Player1 === player.name ||
            game.team2Player2 === player.name);
...
router.post('/:shareCode/games', (0, permissions_1.requireOrganizer)('generate_pairings'), async (req, res) => {
    try {
        const { shareCode } = req.params;
        const { courtName, team1Player1, team1Player2, team1Score, team2Player1, team2Player2, team2Score, winnerTeam, startTime, endTime, duration, sets } = req.body;
        const session = await database_1.prisma.mvpSession.findUnique({
            where: { shareCode },
            include: { games: true }
        });
        ...
        if (sets && Array.isArray(sets)) {
            for (const set of sets) {
                await database_1.prisma.mvpGameSet.create({
                    data: {
                        gameId: game.id,
...
router.put('/:shareCode/courts', rateLimiters.api, (0, permissions_1.requireOrganizer)('edit_session'), async (req, res) => {
    try {
        const { shareCode } = req.params;
...
router.delete('/:shareCode/leave-by-device', async (req, res) => {
    try {
        ...
        if (typeof server_1.io !== 'undefined') {
            server_1.io.to(`session-${shareCode}`).emit('mvp-session-updated', {
                type: 'player-left',
                player: {
                    id: updatedPlayer.id,
...
//# sourceMappingURL=mvpSessions.js.map