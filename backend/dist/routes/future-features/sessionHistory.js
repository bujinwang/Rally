...
const getHistoryValidation = [
    (0, express_validator_1.query)('deviceId').optional().isLength({ min: 1 }).withMessage('Device ID is required for filtering'),
    (0, express_validator_1.query)('filter').optional().isIn(['all', 'organized', 'participated']).withMessage('Invalid filter type'),
    (0, express_validator_1.query)('sortBy').optional().isIn(['date', 'games', 'duration', 'players']).withMessage('Invalid sort field'),
...
const createGameValidation = [
    (0, express_validator_1.param)('sessionId').isLength({ min: 1 }).withMessage('Session ID is required'),
    (0, express_validator_1.body)('courtName').optional().isLength({ max: 50 }).withMessage('Court name too long'),
    (0, express_validator_1.body)('team1Player1').isLength({ min: 1, max: 100 }).withMessage('Team 1 Player 1 name required'),
    (0, express_validator_1.body)('team1Player2').isLength({ min: 1, max: 100 }).withMessage('Team 1 Player 2 name required'),
    (0, express_validator_1.body)('team2Player1').isLength({ min: 1, max: 100 }).withMessage('Team 2 Player 1 name required'),
    (0, express_validator_1.body)('team2Player2').isLength({ min: 1, max: 100 }).withMessage('Team 2 Player 2 name required')
];
const updateGameValidation = [
    (0, express_validator_1.param)('gameId').isLength({ min: 1 }).withMessage('Game ID is required'),
    (0, express_validator_1.body)('team1FinalScore').optional().isInt({ min: 0 }).withMessage('Team 1 score must be non-negative'),
    (0, express_validator_1.body)('team2FinalScore').optional().isInt({ min: 0 }).withMessage('Team 2 score must be non-negative'),
    (0, express_validator_1.body)('winnerTeam').optional().isIn([1, 2]).withMessage('Winner team must be 1 or 2'),
    (0, express_validator_1.body)('status').optional().isIn(['IN_PROGRESS', 'COMPLETED', 'PAUSED', 'CANCELLED']).withMessage('Invalid status')
];
...
router.get('/', optionalAuth, getHistoryValidation, async (req, res) => {
    try {
        ...
        const { deviceId, filter = 'all', sortBy = 'date', limit = 20, offset = 0 } = req.query;
        ...
        const currentUser = (0, auth_1.getCurrentUser)(req);
        if (currentUser && filter !== 'all') {
            ...
            if (filter === 'organized') {
                ...
                const participatedSessions = await database_1.prisma.sessionPlayer.findMany({
            ...
            if (filter === 'organized') {
                whereConditions.ownerDeviceId = deviceId;
            }
            else if (filter === 'participated') {
        ...
        const totalCount = await database_1.prisma.mvpSession.count({
            where: whereConditions
        });
        res.json({
            success: true,
...
router.get('/:sessionId', optionalAuth, async (req, res) => {
    try {
        ...
        const playersWithStats = session.players.map(player => {
            ...
            const playerGames = session.games.filter(game => game.team1Player1 === player.name ||
        ...
        const sessionHistory = {
            ...
            summary: {
                ...
                mostActivePlayer: playersWithStats.reduce((prev, current) => (prev.gamesPlayed > current.gamesPlayed) ? prev : current, playersWithStats[0])?.name || null
            }
        };
        res.json({
            success: true,
...
router.post('/sessions/:sessionId/games', createGameValidation, async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({
        ...
        const { sessionId } = req.params;
        const { courtName, team1Player1, team1Player2, team2Player1, team2Player2 } = req.body;
...
// Update game results
router.put('/games/:gameId', updateGameValidation, async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({
        ...
        const game = await database_1.prisma.mvpGame.update({
            where: { id: gameId },
            data: updateData,
            include: {
                sets: true,
        ...
        if (updateData.status === 'COMPLETED' && updateData.winnerTeam) {
            ...
            if (game.startTime && game.endTime) {
                ...
                await database_1.prisma.mvpGame.update({
                    where: { id: gameId },
...
router.get('/players/:deviceId/stats', optionalAuth, async (req, res) => {
    try {
        ...
        const totalGamesPlayed = sessions.reduce((acc, session) => acc + (session.players[0]?.gamesPlayed || 0), 0);
        const totalWins = sessions.reduce((acc, session) => acc + (session.players[0]?.wins || 0), 0);
        const totalLosses = sessions.reduce((acc, session) => acc + (session.players[0]?.losses || 0), 0);
        const overallWinRate = totalGamesPlayed > 0 ? Math.round((totalWins / totalGamesPlayed) * 100) : 0;
        // Recent performance (last 5 sessions)
        const recentSessions = sessions.slice(-5);
        const recentWins = recentSessions.reduce((acc, session) => acc + (session.players[0]?.wins || 0), 0);
        const recentGames = recentSessions.reduce((acc, session) => acc + (session.players[0]?.gamesPlayed || 0), 0);
