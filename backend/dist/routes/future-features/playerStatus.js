...
const statusRequestValidation = [
    ...
    (0, express_validator_1.body)('action').isIn(['rest', 'leave']).withMessage('Action must be rest or leave'),
    (0, express_validator_1.body)('reason').optional().isLength({ max: 255 }).withMessage('Reason must be less than 255 characters')
];
const approvalValidation = [
    (0, express_validator_1.param)('requestId').isLength({ min: 1 }).withMessage('Request ID is required'),
    (0, express_validator_1.body)('approved').isBoolean().withMessage('Approved must be a boolean'),
    (0, express_validator_1.body)('reason').optional().isLength({ max: 255 }).withMessage('Reason must be less than 255 characters')
];
// Request status change (rest or leave)
router.post('/:playerId/status', statusRequestValidation, async (req, res) => {
    try {
        ...
        const activeGames = await database_1.prisma.mvpGame.findMany({
            where: {
        ...
        const statusRequest = await database_1.prisma.mvpPlayer.update({
            where: { id: playerId },
            data: {
...
router.put('/approve/:requestId', (0, permissions_1.requireOrganizer)('update_player_status'), approvalValidation, async (req, res) => {
    try {
        const errors = (0, express_validator_1.validationResult)(req);
        if (!errors.isEmpty()) {
            return res.status(400).json({
        ...
        const statusHistory = player.statusHistory || [];
...
router.get('/pending/:shareCode', (0, permissions_1.requireOrganizer)('update_player_status'), async (req, res) => {
    try {
...
router.post('/expire-rest/:playerId', async (req, res) => {
    try {
        const { playerId } = req.params;
        const player = await database_1.prisma.mvpPlayer.findUnique({
            where: { id: playerId },
