...
router.get('/:shareCode', async (req, res) => {
    try {
        ...
        const clientFingerprint = req.query.fp;
        // Find the session
      ...
      `);
        ...
        const deviceId = (0, deviceFingerprint_1.generateDeviceFingerprint)(req, clientFingerprint);
 ...
 */
router.get('/:shareCode/player-status', async (req, res) => {
    try {
        const { shareCode } = req.params;
        const clientFingerprint = req.query.fp;
 ...
 */
router.delete('/:shareCode/leave', async (req, res) => {
    try {
        const { shareCode } = req.params;
        const clientFingerprint = req.body.deviceFingerprint || req.query.fp;
        if (!clientFingerprint) {
            return res.status(400).json({
                success: false,
 ...
 */
function generateSessionHTML(session, existingPlayer, deviceId) {
    const playerCount = session.players.filter((p) => p.status === 'ACTIVE').length;
    ...
    <body>
      ...
      <script>
        ...
        ${deviceFingerprint_1.CLIENT_FINGERPRINT_SCRIPT}
