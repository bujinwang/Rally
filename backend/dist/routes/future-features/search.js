...
const searchValidation = [
    (0, express_validator_1.query)('q').isLength({ min: 1 }).withMessage('Search query is required'),
    (0, express_validator_1.query)('type').optional().isIn(['sessions', 'players', 'all']).withMessage('Invalid search type'),
    (0, express_validator_1.query)('limit').optional().isInt({ min: 1, max: 50 }).withMessage('Limit must be between 1 and 50'),
    (0, express_validator_1.query)('includeCompleted').optional().isBoolean().withMessage('includeCompleted must be boolean')
];
...
router.get('/', searchValidation, async (req, res) => {
    try {
        ...
        const { q: searchQuery, type = 'all', limit = 20, includeCompleted = false } = req.query;
        ...
        if (results.totalResults === 0) {
            ...
            const recentSessions = await database_1.prisma.mvpSession.findMany({
                where: { status: 'ACTIVE' },
            ...
            suggestions = [
                ...
                ...recentSessions.map(s => s.location).filter((loc) => Boolean(loc))
            ];
        }
        res.json({
            success: true,
...
router.get('/suggestions', async (req, res) => {
    try {
        ...
        const sessionSuggestions = await database_1.prisma.mvpSession.findMany({
            where: {
                OR: [
        ...
        const playerSuggestions = await database_1.prisma.mvpPlayer.findMany({
            where: {
                name: { startsWith: query, mode: 'insensitive' },
