...
router.post('/detailed', async (req, res) => {
    try {
        const { sessionId, player1Id, player2Id, sets, scoringSystem = '21_POINT', bestOfGames = 3, deviceId, courtName } = req.body;
        ...
        // Update rankings
        try {
            await rankingService_1.rankingService.updateRatingsAfterMatch(match.id);
        }
        catch (rankingError) {
...
router.get('/detailed/:matchId', async (req, res) => {
    try {
...
async function updatePlayerStatistics(playerId, isWinner) {
...
async function getUpdatedStatistics(playerIds) {
