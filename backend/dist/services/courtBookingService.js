...
exports.CourtBookingService = void 0;
...
class CourtBookingService {
    ...
    static async searchCourts(filters) {
        ...
        if (filters.priceRange) {
            courts = courts.filter(c => c.basePrice >= filters.priceRange.min &&
    ...
    static async checkCourtAvailability(courtId, startTime, endTime) {
        ...
        const conflicts = bookingStore.filter(booking => booking.courtId === courtId &&
            booking.status !== courts_1.BookingStatus.CANCELLED &&
            booking.status !== courts_1.BookingStatus.NO_SHOW &&
            ((booking.startTime <= startTime && booking.endTime > startTime) ||
                (booking.startTime < endTime && booking.endTime >= endTime) ||
                (booking.startTime >= startTime && booking.endTime <= endTime)));
        // Check for maintenance
        const maintenanceConflicts = maintenanceStore.filter(maint => maint.courtId === courtId &&
     ...
     */
    static async calculateBookingPrice(courtId, startTime, endTime, userType, membershipLevel) {
        ...
        const applicableRules = pricingStore.filter(rule => rule.courtId === courtId &&
            rule.isActive &&
        ...
        for (const rule of applicableRules) {
            if (rule.ruleType === courts_1.PricingRuleType.BASE_RATE) {
     ...
     */
    static async createBooking(request, userId) {
        // Validate court exists
        const court = courtStore.find(c => c.id === request.courtId);
        if (!court)
            throw new Error('Court not found');
        // Check availability
        const isAvailable = await this.checkCourtAvailability(request.courtId, request.startTime, request.endTime);
        if (!isAvailable)
            throw new Error('Court is not available for the requested time');
        // Calculate price
        const totalPrice = await this.calculateBookingPrice(request.courtId, request.startTime, request.endTime);
        ...
        const booking = {
            ...
            duration,
            status: courts_1.BookingStatus.PENDING,
            bookingType: request.bookingType || courts_1.BookingType.CASUAL,
            totalPrice,
            currency: court.currency,
            paymentStatus: courts_1.PaymentStatus.PENDING,
        ...
        await this.createNotification({
            ...
            type: courts_1.CourtNotificationType.BOOKING_CONFIRMED,
     ...
     */
    static async updateBookingStatus(bookingId, status, userId) {
        ...
        if (status === courts_1.BookingStatus.CANCELLED) {
            booking.cancelledAt = new Date();
        }
        // Create notification
        const notificationType = status === courts_1.BookingStatus.CANCELLED
            ? courts_1.CourtNotificationType.BOOKING_CANCELLED
            : courts_1.CourtNotificationType.BOOKING_CONFIRMED;
        await this.createNotification({
            userId: booking.userId,
            courtId: booking.courtId,
            bookingId: booking.id,
            type: notificationType,
            title: status === courts_1.BookingStatus.CANCELLED ? 'Booking Cancelled' : 'Booking Updated',
     ...
     */
    static async confirmBookingPayment(bookingId, paymentIntentId) {
        const booking = bookingStore.find(b => b.id === bookingId);
        if (!booking)
            throw new Error('Booking not found');
        ...
        if (paymentIntent.status === 'succeeded') {
            booking.status = courts_1.BookingStatus.CONFIRMED;
            booking.paymentStatus = courts_1.PaymentStatus.PAID;
        }
        else if (paymentIntent.status === 'canceled') {
            booking.status = courts_1.BookingStatus.CANCELLED;
            booking.paymentStatus = courts_1.PaymentStatus.FAILED;
        }
        else {
            // Payment still processing
            booking.paymentStatus = courts_1.PaymentStatus.PENDING;
        }
        booking.updatedAt = new Date();
        // Create notification
        ...
        let priority = 'MEDIUM';
        if (booking.status === courts_1.BookingStatus.CONFIRMED) {
        ...
        else if (booking.status === courts_1.BookingStatus.CANCELLED) {
        ...
        await this.createNotification({
            ...
            courtId: booking.courtId,
            bookingId: booking.id,
            type: courts_1.CourtNotificationType.PAYMENT_DUE,
            title: notificationTitle,
            message: notificationMessage,
            priority: priority,
            read: false,
            actionRequired: booking.status === courts_1.BookingStatus.PENDING,
        });
        return booking;
    }
    /**
     ...
     */
    static async processBookingRefund(bookingId, reason) {
        const booking = bookingStore.find(b => b.id === bookingId);
        if (!booking)
            throw new Error('Booking not found');
        ...
        if (booking.paymentStatus !== courts_1.PaymentStatus.PAID) {
        ...
        // Update booking status
        booking.status = courts_1.BookingStatus.CANCELLED;
        booking.paymentStatus = courts_1.PaymentStatus.REFUNDED;
        ...
        await this.createNotification({
            ...
            courtId: booking.courtId,
            bookingId: booking.id,
            type: courts_1.CourtNotificationType.BOOKING_CANCELLED,
     ...
     * Get user's bookings
     */
    static async getUserBookings(userId, status, upcomingOnly = false) {
     ...
     */
    static async getCourtBookings(courtId, startDate, endDate) {
        return bookingStore.filter(booking => booking.courtId === courtId &&
     ...
     */
    static async createMaintenance(request) {
        const maintenance = {
        ...
        const court = courtStore.find(c => c.id === request.courtId);
        if (court) {
            court.status = courts_1.CourtStatus.MAINTENANCE;
     ...
     */
    static async getCourtUsageStats(courtId, startDate, endDate) {
        const bookings = bookingStore.filter(booking => booking.courtId === courtId &&
            booking.createdAt >= startDate &&
            booking.createdAt <= endDate &&
            booking.status === courts_1.BookingStatus.COMPLETED);
     ...
     */
    static async getVenueRevenueReport(venueId, startDate, endDate) {
        const venueCourts = courtStore.filter(court => court.venueId === venueId);
        const courtIds = venueCourts.map(court => court.id);
        const bookings = bookingStore.filter(booking => courtIds.includes(booking.courtId) &&
            booking.createdAt >= startDate &&
            booking.createdAt <= endDate &&
            booking.paymentStatus === courts_1.PaymentStatus.PAID);
        const totalRevenue = bookings.reduce((sum, booking) => sum + booking.totalPrice, 0);
        ...
        return {
            ...
            totalBookings,
            averageBookingValue,
            revenueByCourtType: {}, // Would need aggregation logic
            revenueByBookingType: {}, // Would need aggregation logic
            revenueByDayOfWeek: {}, // Would need aggregation logic
            revenueByHour: {}, // Would need aggregation logic
            period: { start: startDate, end: endDate },
        };
    }
}
exports.CourtBookingService = CourtBookingService;
//# sourceMappingURL=courtBookingService.js.map