...
class StatisticsService {
     ...
     */
    async getPlayerStatistics(playerId, filters = {}) {
        try {
     ...
     */
    calculateStreaks(matches, playerId) {
     ...
     */
    calculatePerformanceRating(winRate, matchesPlayed, winStreak) {
        if (matchesPlayed === 0)
    ...
    async getLeaderboard(filters = {}) {
        try {
            ...
            const leaderboard = playerStats.map((stats, index) => ({
                rank: index + 1,
    ...
    async getSessionStatistics(sessionId) {
        try {
            ...
            const playerMatchCounts = await Promise.all(session.players.map(async (player) => ({
                name: player.name,
            ...
            const mostActivePlayer = playerMatchCounts.reduce((max, player) => player.count > max.count ? player : max, { name: '', count: 0 }).name;
            return {
                sessionId,
     ...
     */
    async getPlayerComparison(playerIds, filters = {}) {
        const comparisons = [];
     ...
     */
    async getPerformanceTrends(playerId, days = 30) {
        try {
