...
exports.TournamentAnalyticsService = void 0;
...
class TournamentAnalyticsService {
     ...
     */
    static async calculateParticipationMetrics(tournamentId) {
     ...
     */
    static async calculateBracketEfficiency(tournamentId) {
        const tournament = await prisma.tournament.findUnique({
            where: { id: tournamentId },
            include: {
                rounds: {
     ...
     */
    static async trackPlayerRankingChanges(tournamentId) {
        const tournament = await prisma.tournament.findUnique({
            where: { id: tournamentId },
            include: {
                players: {
     ...
     */
    static async compareTournamentFormats(tournamentIds) {
        ...
        Object.keys(comparisons).forEach(format => {
            ...
            const avgDuration = stats.reduce((sum, s) => sum + s.avgMatchDuration, 0) / stats.length;
            const avgEfficiency = stats.reduce((sum, s) => sum + s.bracketEfficiency, 0) / stats.length;
            const avgParticipants = stats.reduce((sum, s) => sum + s.participants, 0) / stats.length;
            comparisons[format] = {
...
exports.TournamentAnalyticsService = TournamentAnalyticsService;
//# sourceMappingURL=tournamentAnalyticsService.js.map