...
class AnalyticsService {
     ...
     */
    static async updatePlayerAnalytics(playerId) {
        try {
      ...
      `;
            const matches = await databaseUtils_1.DatabaseUtils.executeRawQuery(matchesQuery, [playerId]);
      ...
      `;
            const sessionsResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(sessionsQuery, [playerId]);
            ...
            const playerNameResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(playerNameQuery, [playerId]);
        ...
        WHERE player_name = $1
      `;
            const tournamentsResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(tournamentsQuery, [playerName]);
      ...
      `;
            const friendsResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(friendsQuery, [playerId]);
      ...
      `;
            const challengesSentResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(challengesSentQuery, [playerId]);
      ...
      `;
            const challengesAcceptedResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(challengesAcceptedQuery, [playerId]);
      ...
      `;
            const achievementsResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(achievementsQuery, [playerId]);
      ...
      `;
            const badgesResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(badgesQuery, [playerId]);
        ...
        WHERE player_id = $1 AND is_completed = true
      `;
            const totalPointsResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(totalPointsQuery, [playerId]);
      ...
      `;
            const notificationsResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(notificationsReadQuery, [playerId]);
            const notificationsRead = parseInt(notificationsResult[0].count) || 0;
            const messagesQuery = `
        SELECT COUNT(*) as count FROM messages WHERE sender_id = $1
      `;
            const messagesResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(messagesQuery, [playerId]);
        ...
        DO UPDATE SET
          ...
          updated_at = NOW()
      `;
            await databaseUtils_1.DatabaseUtils.executeRawQuery(upsertQuery, [
    ...
    static async updateSessionAnalytics(sessionId) {
        ...
        GROUP BY s.id
      `;
            const sessionResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(sessionQuery, [sessionId]);
        ...
        GROUP BY m.id
      `;
            const matches = await databaseUtils_1.DatabaseUtils.executeRawQuery(matchesQuery, [sessionId]);
            const totalMatches = matches.length;
            ...
            const matchDurations = matches
                .filter((match) => match.start_time && match.end_time)
                .map((match) => {
            ...
            const averageMatchDuration = matchDurations.length > 0
                ? matchDurations.reduce((sum, duration) => sum + duration, 0) / matchDurations.length
                : 0;
      ...
      `;
            const activePlayersResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(activePlayersQuery, [sessionId]);
     ...
     */
    static async updateTournamentAnalytics(tournamentId) {
        try {
        ...
        GROUP BY t.id
      `;
            const tournamentResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(tournamentQuery, [tournamentId]);
        ...
        GROUP BY tm.id
      `;
            const matches = await databaseUtils_1.DatabaseUtils.executeRawQuery(matchesQuery, [tournamentId]);
            const totalMatches = matches.length;
            // Calculate completion rate
            const completedMatches = matches.filter((match) => match.status === 'COMPLETED').length;
            const completionRate = totalMatches > 0 ? (completedMatches / totalMatches) * 100 : 0;
      ...
      `;
            const confirmedPlayersResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(confirmedPlayersQuery, [tournamentId]);
            const confirmedPlayers = parseInt(confirmedPlayersResult[0].count) || 0;
            const activePlayersQuery = `
        SELECT COUNT(*) as count FROM tournament_players
        WHERE tournament_id = $1 AND status != 'WITHDRAWN'
      `;
            const activePlayersResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(activePlayersQuery, [tournamentId]);
            const activePlayers = parseInt(activePlayersResult[0].count) || 0;
     ...
     */
    static async getSessionAnalyticsDashboard(startDate, endDate, filters) {
        try {
     ...
     */
    static async getSessionTrends(startDate, endDate, filters) {
        try {
      ...
      `;
            const trends = await databaseUtils_1.DatabaseUtils.executeRawQuery(trendsQuery, params);
            const totalSessions = trends.reduce((sum, day) => sum + parseInt(day.session_count), 0);
            return {
                totalSessions,
                data: trends.map((day) => ({
                    date: day.date,
     ...
     * Get participation analysis
     */
    static async getParticipationAnalysis(startDate, endDate, filters) {
        try {
            const dateFilter = startDate && endDate ? `AND s.created_at >= $1 AND s.created_at <= $2` : '';
            const params = startDate && endDate ? [startDate, endDate] : [];
      ...
      `;
            const participationData = await databaseUtils_1.DatabaseUtils.executeRawQuery(participationQuery, params);
        ...
        WHERE s.status = 'COMPLETED' ${dateFilter}
      `;
            const uniquePlayersResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(uniquePlayersQuery, params);
        ...
        WHERE s.status = 'COMPLETED' ${dateFilter}
      `;
            const avgAttendanceResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(avgAttendanceQuery, params);
            const avgAttendance = parseFloat(avgAttendanceResult[0].avg_attendance) || 0;
            return {
                totalUniquePlayers,
                avgAttendance,
                frequencyDistribution: participationData.map((row) => ({
     ...
     */
    static async getGeographicDistribution(startDate, endDate, filters) {
        try {
            const dateFilter = startDate && endDate ? `AND s.created_at >= $1 AND s.created_at <= $2` : '';
            const params = startDate && endDate ? [startDate, endDate] : [];
            const geoQuery = `
        ...
        LIMIT 20
      `;
            const geoData = await databaseUtils_1.DatabaseUtils.executeRawQuery(geoQuery, params);
            return {
                topLocations: geoData.map((row) => ({
     ...
     */
    static async getSessionTypeAnalytics(startDate, endDate, filters) {
        try {
            const dateFilter = startDate && endDate ? `AND s.created_at >= $1 AND s.created_at <= $2` : '';
            const params = startDate && endDate ? [startDate, endDate] : [];
            const typeQuery = `
      ...
      `;
            const typeData = await databaseUtils_1.DatabaseUtils.executeRawQuery(typeQuery, params);
     ...
     * Get peak usage patterns
     */
    static async getPeakUsagePatterns(startDate, endDate, filters) {
        try {
            const dateFilter = startDate && endDate ? `AND s.created_at >= $1 AND s.created_at <= $2` : '';
            const params = startDate && endDate ? [startDate, endDate] : [];
        ...
        LIMIT 5
      `;
            const peakHoursData = await databaseUtils_1.DatabaseUtils.executeRawQuery(peakHoursQuery, params);
            const popularTimes = peakHoursData.map((row) => ({
     ...
     */
    static async exportAnalyticsData(startDate, endDate, format = 'json') {
        try {
     ...
     */
    static async trackAnalyticsEvent(type, entityId, userId, data) {
        try {
            const eventData = {
                type,
                entityId,
        ...
        VALUES ($1, $2, $3, $4, $5)
      `;
            await databaseUtils_1.DatabaseUtils.executeRawQuery(insertQuery, [
                eventData.type,
                eventData.entityId,
                eventData.userId,
     ...
     */
    static async generateSystemAnalytics(date = new Date()) {
        try {
        ...
        WHERE updated_at >= $1
      `;
            const activeUsersResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(activeUsersQuery, [thirtyDaysAgo]);
        ...
        WHERE status = 'ACTIVE'
      `;
            const totalSessionsResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(totalSessionsQuery, []);
            const totalSessions = parseInt(totalSessionsResult[0].count) || 0;
            const sessionsCreatedTodayQuery = `
        SELECT COUNT(*) as count FROM mvp_sessions
        WHERE created_at >= $1 AND created_at <= $2
      `;
            const sessionsCreatedResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(sessionsCreatedTodayQuery, [startOfDay, endOfDay]);
      ...
      `;
            const matchesResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(matchesTodayQuery, [startOfDay, endOfDay]);
      ...
      `;
            const friendRequestsResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(friendRequestsQuery, [startOfDay, endOfDay]);
            const friendRequestsSent = parseInt(friendRequestsResult[0].count) || 0;
            const challengesQuery = `
        SELECT COUNT(*) as count FROM challenges
        WHERE sent_at >= $1 AND sent_at <= $2
      `;
            const challengesResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(challengesQuery, [startOfDay, endOfDay]);
            const challengesCreated = parseInt(challengesResult[0].count) || 0;
            const messagesQuery = `
        SELECT COUNT(*) as count FROM messages
        WHERE sent_at >= $1 AND sent_at <= $2
      `;
            const messagesResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(messagesQuery, [startOfDay, endOfDay]);
            const messagesSent = parseInt(messagesResult[0].count) || 0;
      ...
      `;
            const achievementsResult = await databaseUtils_1.DatabaseUtils.executeRawQuery(achievementsQuery, [startOfDay, endOfDay]);
            const achievementsUnlocked = parseInt(achievementsResult[0].count) || 0;
     ...
     */
    static async getPlayerLeaderboard(limit = 10) {
        try {
        ...
        LIMIT $1
      `;
            const leaderboard = await databaseUtils_1.DatabaseUtils.executeRawQuery(leaderboardQuery, [limit]);
     ...
     */
    static async getPlayerPerformanceTrends(playerId, days = 30) {
        try {
    ...
    static async getPlayerName(playerId) {
        ...
        const result = await databaseUtils_1.DatabaseUtils.executeRawQuery(playerQuery, [playerId]);
        return result[0]?.name || 'Unknown Player';
    }
    static calculateSkillRating(matches, playerId) {
    ...
    static calculateCurrentStreak(matches, playerId) {
        if (matches.length === 0)
            return 0;
        // Sort matches by date (most recent first)
        const sortedMatches = matches.sort((a, b) => new Date(b.recordedAt).getTime() - new Date(a.recordedAt).getTime());
        let streak = 0;
    ...
    static async getBestStreak(playerId) {
        ...
        const result = await databaseUtils_1.DatabaseUtils.executeRawQuery(analyticsQuery, [playerId]);
        return Math.abs(parseInt(result[0]?.current_streak) || 0);
    }
    static async calculateAverageGameDuration(playerId) {
    ...
    `;
        const games = await databaseUtils_1.DatabaseUtils.executeRawQuery(gamesQuery, [playerName]);
        const durations = games
            .filter((game) => game.start_time && game.end_time)
            .map((game) => {
    ...
    static async calculateHoursPlayed(playerId) {
      ...
      WHERE team1_player1 = $1 OR team1_player2 = $1 OR team2_player1 = $1 OR team2_player2 = $1
    `;
        const result = await databaseUtils_1.DatabaseUtils.executeRawQuery(totalGamesQuery, [playerName]);
