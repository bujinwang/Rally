...
class TournamentBracketService {
     ...
     */
    async generateSingleEliminationBracket(options) {
     ...
     */
    async generateDoubleEliminationBracket(options) {
        const { tournamentId, players } = options;
        const totalPlayers = players.length;
     ...
     */
    async generateRoundRobinBracket(options) {
        const { tournamentId, players } = options;
        const totalPlayers = players.length;
     ...
     */
    async generateSwissBracket(options) {
        const { tournamentId, players } = options;
        const totalPlayers = players.length;
        ...
        for (let round = 1; round <= totalRounds; round++) {
            ...
            for (let i = 0; i < availablePlayers.length; i++) {
                if (pairedPlayers.has(availablePlayers[i].id))
                    continue;
     ...
     */
    sortPlayersForBracket(players, randomizeSeeding = false) {
        if (randomizeSeeding) {
            // Random seeding
     ...
     */
    async updateMatchResult(tournamentId, matchId, winnerId, score) {
        try {
            // Update match result
     ...
     * Advance winner to next round
     */
    async advanceWinnerToNextRound(tournamentId, matchId, winnerId) {
        try {
            const match = await prisma.tournamentMatch.findUnique({
                where: { id: matchId },
                include: { round: true },
     ...
     */
    async getBracketState(tournamentId) {
        try {
...
exports.tournamentBracketService = new TournamentBracketService();
exports.default = exports.tournamentBracketService;
//# sourceMappingURL=tournamentBracketService.js.map