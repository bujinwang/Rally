...
exports.AIPairingService = void 0;
const client_1 = require("@prisma/client");
const cacheService_1 = require("./cacheService");
const prisma = new client_1.PrismaClient();
class AIPairingService {
    /**
     * Generate AI-powered pairing suggestions for a session
     */
    static async generateAISuggestions(sessionId, playerIds, options = {}) {
        const startTime = Date.now();
        const { maxSuggestions = 5, includeHistoricalData = true, preferenceWeight = 0.3 } = options;
        try {
            ...
            const scoredPairings = await Promise.all(possiblePairings.map(pairing => this.scorePairing(pairing, modelParams, {
            ...
            await cacheService_1.cacheService.set(cacheKey, result, this.CACHE_TTL);
     ...
     */
    static async getPlayersWithAIData(sessionId, playerIds) {
        ...
        const playersWithAIData = players.map(player => ({
            id: player.id,
            name: player.name,
        ...
        // Cache for 5 minutes
        await cacheService_1.cacheService.set(cacheKey, playersWithAIData, this.CACHE_TTL);
        return playersWithAIData;
    }
    /**
     * Calculate skill level if not set (fallback algorithm)
     */
    static calculateSkillLevel(player) {
     ...
     */
    static generatePossiblePairings(players) {
        const pairings = [];
     ...
     */
    static async scorePairing([player1, player2], modelParams, options) {
        const factors = {
        ...
        const weights = modelParams?.skillWeights || {
            skillMatch: 0.5,
            preferenceMatch: 0.3,
            historicalCompatibility: 0.2
        };
        const confidence = (factors.skillMatch * weights.skillMatch +
     ...
     */
    static calculateSkillMatch(player1, player2) {
     ...
     */
    static calculatePreferenceMatch(player1, player2, weight) {
     ...
     */
    static async calculateHistoricalCompatibility(player1, player2) {
        // Check if they've played together before
        const pastPairings = player1.pairingHistory.filter(history => history.partnerId === player2.id);
     ...
     */
    static generatePairingReason(factors, confidence) {
        const reasons = [];
     ...
     */
    static async getActiveModelParameters() {
        try {
            ...
            if (activeModel) {
                await cacheService_1.cacheService.set(cacheKey, activeModel, 3600); // Cache for 1 hour
            }
            return activeModel;
        }
        catch (error) {
     ...
     */
    static async recordPairingFeedback(sessionId, playerId, partnerId, feedback, aiSuggested = false) {
        try {
            ...
            await cacheService_1.cacheService.delete(`players-ai-data:${sessionId}`);
     ...
     */
    static async updatePlayerSkillLevels(sessionId) {
        try {
            const players = await prisma.mvpPlayer.findMany({
                where: { sessionId },
                select: {
...
exports.AIPairingService = AIPairingService;
AIPairingService.CACHE_TTL = 300; // 5 minutes
AIPairingService.CONFIDENCE_THRESHOLD = 0.7;
//# sourceMappingURL=aiPairingService.js.map