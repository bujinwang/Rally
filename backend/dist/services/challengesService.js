...
exports.challengesService = exports.ChallengesService = void 0;
...
class ChallengesService {
     ...
     */
    async respondToChallenge(challengeId, userId, accept) {
    ...
    async getChallengeStats(userId) {
        ...
        return {
            sentCount: Array.isArray(sentCount) ? parseInt(sentCount[0].count) || 0 : 0,
            receivedCount: Array.isArray(receivedCount) ? parseInt(receivedCount[0].count) || 0 : 0,
            acceptedCount: Array.isArray(acceptedCount) ? parseInt(acceptedCount[0].count) || 0 : 0,
            completedCount: Array.isArray(completedCount) ? parseInt(completedCount[0].count) || 0 : 0
        };
    }
    /**
     * Get pending challenges count for a user
     */
    async getPendingChallengesCount(userId) {
        const result = await prisma.$queryRaw `
      SELECT COUNT(*) as count FROM challenges
      WHERE challenged_id = ${userId} AND status = 'PENDING'
    `;
        return Array.isArray(result) ? parseInt(result[0].count) || 0 : 0;
    }
    /**
...
//# sourceMappingURL=challengesService.js.map