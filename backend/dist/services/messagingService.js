...
class MessagingService {
     ...
     */
    async getThreadMessages(threadId, userId, limit = 50, offset = 0) {
     ...
     */
    async getThreadUnreadCount(threadId, userId) {
        const result = await prisma.$queryRaw `
      SELECT COUNT(*) as count FROM messages m
      JOIN message_threads mt ON m.thread_id = mt.id
      WHERE m.thread_id = ${threadId}
    ...
    `;
        ...
        const participants = threadResult[0].participants;
    ...
    `;
        ...
        const currentParticipants = threadResult[0].participants;
     ...
     * Get thread details
     */
    async getThreadDetails(threadId, userId) {
        const threadResult = await prisma.$queryRaw `
     ...
     */
    async searchMessages(threadId, userId, query, limit = 20) {
        // Verify user is a participant
        const threadResult = await prisma.$queryRaw `
      SELECT * FROM message_threads
      WHERE id = ${threadId} AND ${userId} = ANY(participants)
