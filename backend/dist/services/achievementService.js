...
class AchievementService {
     ...
     */
    async checkAndUpdateAchievements(playerId, trigger) {
        const achievements = await this.getActiveAchievements();
        const progressUpdates = [];
        for (const achievement of achievements) {
            if (achievement.triggerType === trigger.type) {
                const progress = await this.updateAchievementProgress(playerId, achievement.id, trigger);
                if (progress) {
     ...
     */
    async updateAchievementProgress(playerId, achievementId, trigger) {
    ...
    `;
      ...
      `;
        ...
        const newProgress = this.calculateProgress(playerAchievement.progress, JSON.parse(achievement.trigger_value || '{}'), trigger);
     ...
     */
    async getAchievementWithBadge(achievementId) {
     ...
     */
    calculateProgress(currentProgress, triggerConfig, trigger) {
        switch (trigger.type) {
            case achievement_1.AchievementTriggerType.MATCH_WIN:
                return currentProgress + (trigger.data?.wins || 1);
            case achievement_1.AchievementTriggerType.MATCH_PLAY:
                return currentProgress + (trigger.data?.matchesPlayed || 1);
            case achievement_1.AchievementTriggerType.TOURNAMENT_WIN:
                return currentProgress + (trigger.data?.tournamentsWon || 1);
            case achievement_1.AchievementTriggerType.TOURNAMENT_PARTICIPATE:
                return currentProgress + (trigger.data?.tournamentsParticipated || 1);
            case achievement_1.AchievementTriggerType.STREAK:
                return Math.max(currentProgress, trigger.data?.currentStreak || 0);
            case achievement_1.AchievementTriggerType.PERFECT_GAME:
                return currentProgress + (trigger.data?.perfectGames || 1);
            case achievement_1.AchievementTriggerType.SOCIAL_FRIEND:
                return currentProgress + (trigger.data?.friendsAdded || 1);
            case achievement_1.AchievementTriggerType.SESSION_HOST:
                return currentProgress + (trigger.data?.sessionsHosted || 1);
            case achievement_1.AchievementTriggerType.SKILL_LEVEL:
                return trigger.data?.currentSkillLevel || currentProgress;
            case achievement_1.AchievementTriggerType.TIME_PLAYED:
     ...
     */
    async createAchievementRewards(playerId, achievementId, achievement) {
        const rewards = [];
        // Points reward
