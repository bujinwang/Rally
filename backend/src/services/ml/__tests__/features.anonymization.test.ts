...
/**
 * The forbidden-key pattern from design §6 #10, expressed with word boundaries.
 *
 * NOTE: the design states the literal regex `/name|deviceId|userId|email|playerId/i`.
 * Applied verbatim it matches the descriptor's legitimate `FeatureSpec.names`
 * schema-label key (`names` contains the substring `name`), which the design's
 * own data model requires. The intent is clearly to ban *PII* keys, so we anchor
 * each alternative with `\b` — `\bname\b` does not match `names`, but still
 * catches a real `name` field. Reported to team-lead as a design/regex conflict.
 */
const PII_KEY = /\b(name|deviceId|userId|email|playerId)\b/i;
...
describe('Story 6.6 — anonymization (#10)', () => {
  ...
  it('toFeatureRow whitelists only the declared feature names', () => {
    ...
    expect(JSON.stringify(row)).not.toMatch(PII_KEY);
  });
  ...
  it('demand feature rows contain no PII keys', () => {
    ...
    for (const key of Object.keys(row)) expect(key).not.toMatch(PII_KEY);
  ...
  it('churn feature rows and the churn descriptor contain no PII keys', async () => {
    ...
    // The serialized descriptor must not leak the identity or device string.
    const serialized = JSON.stringify(dataset.descriptor);
